...
const subCommunities: Record<string, string[]> = {
  ...
  "Sustainability": ["Energy Efficiency", "Occupational Health", "Industrial Safety", "Environment"],
};
...
export function RegisterPage({ email, onRegistrationComplete }: RegisterPageProps) {
  ...
  const sendPhoneOtp = async () => {
    try {
      ...
      await AuthAPI.requestOtp(phone, "registration_phone");
    } catch (e: any) {
  ...
  const verifyPhoneOtp = async () => {
    try {
      ...
      await AuthAPI.verifyOtp(phone, otp, "registration_phone");
